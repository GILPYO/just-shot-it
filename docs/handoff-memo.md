...
- 보스 시스템 (10레벨마다 스폰)
- 난이도 스케일링 (계단식 HP + 숨돌리기)
- 게임오버 화면 + 메인메뉴

### ✅ Phase 3 진행 중 — 오늘(9/9) 완료한 것
- 스프라이트 에셋 통합 (PixelLab 제작, 96×96, 8방향 × 9프레임)
  - 플레이어: hero80_walk8.png
  - 학생좀비: zombie80_walk8.png
  - 안전모좀비: hardhat80_walk8.png (preload 완료, 스폰 코드 50% 랜덤)
  - 뚱좀비(보스): fat80_walk8.png + fat80_roar8_1.png + JSON (에셋만 준비)
- 8방향 프레임 선택 (OCT2ROW 매핑 + walkFrame 애니메이션)
- 총구(muzzle) 시스템: JSON muzzle 데이터로 방향별/프레임별 총구 좌표에서 총알 발사
- 좀비 hitbox 조정: setSize(28, 50) + setOffset(34, 10) — 머리~다리 커버
- 플레이어 hitbox: setSize(20, 20) + setOffset(38, 50)
- 불필요 에셋 삭제 (hero.png, react.svg, vite.svg)

### 🔧 다음에 할 것

#### 사운드 (다음 우선순위!)
- 권총 발사음 (0.3~0.5초) — freesound.org에서 다운
- 탄피 낙하음 (0.2~0.3초)
- 재장전 소리 (~1초)
- 좀비 신음, 발소리 등
- `public/assets/sounds/` 폴더에 배치

#### 뚱좀비 보스 구현
- fat80_walk8.png (걷기) + fat80_roar8_1.png (포효) 에셋 준비됨
- fat80_roar.json: 4방향 소스 + snap 매핑, glow(발광), widthAmp(팽창) 데이터
- 보스 등장 시 포효 애니메이션 → 걷기로 전환
- 시야 차단 역할 (뒤의 좀비가 안 보임)

#### 기타 TODO
- 레벨별 좀비 종류 출현 기획 (현재는 랜덤 50%)
- 사망/피격 이펙트 (흰 실루엣 + 회전 + 축소)
- 피 데칼 (바닥에 영구 배치)
- 타일맵/배경
